...
public final class LangIdV3 implements ILangIdClassifier {
  ...
  public DetectedLanguage classify(boolean normalizeConfidence) {
    ...
    for (int i = 1; i < probs.length; i++) {
      if (probs[i] > max && Legal.contains(model.langClasses[i])) {
        c = i;
        max = probs[i];
        top = new Pair(max, model.langClasses[i]);
        
      }
      else if(Legal.contains(model.langClasses[i])){
    	  
    	  values.put(model.langClasses[i], probs[i]);
      }
    }
    ...
    return new DetectedLanguage(model.langClasses[c], Math.abs( max));    
  }
